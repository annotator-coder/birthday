# GS칼텍스 브랜드 보이스 가이드

**버전:** 1.0 | **최종 업데이트:** 2026-05-15

## 핵심 톤 3가지
- **전문적이되 친근하게:** 에너지 전문 기업이지만 일반 독자도 이해할 수 있게
- **사실 중심·과장 없이:** 수치와 사실로 말하고, 과도한 미사여구 피함
- **미래 지향적·행동하는:** 전환(Transformation) 기업으로서 능동적 어조

## 문체 규칙
- 문장: 2~3줄 이내. 핵심 먼저, 근거 나중
- 호칭: 독자를 직접 지칭할 때 "여러분" 사용 (고객님 X)
- 수치: 가능하면 구체적 수치 제시 (예: "크게 성장" → "YoY 23% 성장")
- 영어: 에너지·기술 전문 용어(ESG, LNG, Hydrogen, DAX 등)는 영어 사용 가능, 단 괄호 안에 한국어 병기. 일반 단어의 영어화는 지양.

## 절대 쓰지 않는 표현
- "업계 최고", "독보적", "압도적" (근거 없는 최상급)
- "~할 것입니다" 남발 (불확실성 과다 노출)
- "노력하겠습니다" (행동으로 보여줄 것)

## 채널별 변형
| 채널 | 어조 | 길이 | 이모지 | 예시 문구 |
|------|------|------|--------|---------|
| LinkedIn | 격식체, 인사이트 강조 | 150~300자 | 최소화 | "GS칼텍스는 2025년 수소 공급 용량을 전년比 40% 확대했습니다." |
| Instagram | 구어체, 감성적 | 50~100자 (본문 캡션 기준) | 1~3개 허용 | "에너지가 바뀌면 일상도 바뀌어요. 우리가 그 변화를 만들어갑니다 ⚡" |
| 보도자료 | 3인칭, 팩트 중심 | 제한 없음 | 없음 | "GS칼텍스(대표이사 허세홍)는 15일 여수공장에서…" |
| 사내 콘텐츠 | 친근한 경어체 | 자유 | 상황에 따라 | "이번 달 G식노트, 보셨나요? 에너지 전환의 핵심을 짚었습니다." |
| 카드뉴스 | 간결·임팩트 | 15~25자/장 | 없음 | "수소, 이제 현실입니다" |

## 검증 방법
기존 콘텐츠를 이 가이드 기준으로 평가할 때 사용하는 프롬프트:

```
아래 콘텐츠를 GS칼텍스 브랜드 보이스 가이드에 맞게 평가해줘.
잘 지켜진 부분과 위반된 부분을 각각 지적하고, 수정 버전을 제안해줘.

[브랜드 보이스 요약]
GS칼텍스는 국내 대표 에너지 기업으로, 톤은 전문적이면서 친근하다.
과장 없이 사실과 수치로 이야기하고, 에너지 전환 시대를 선도하는
능동적 어조를 유지한다. 문장은 짧고 명확하게.

[평가할 콘텐츠]
(여기에 평가할 콘텐츠 붙여넣기)
```

## AI 프롬프트 삽입용 요약 블록
> GS칼텍스는 국내 대표 에너지 기업으로, 톤은 전문적이면서 친근하다.
> 과장 없이 사실과 수치로 이야기하고, 에너지 전환 시대를 선도하는
> 능동적 어조를 유지한다. 문장은 짧고 명확하게.
> 금지 표현: "업계 최고", "독보적", "압도적", "노력하겠습니다".
